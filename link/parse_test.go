...
func slicesEqual(expected, actual []Link) bool {
...
<body>
	...
	if link, _ := Parse(strings.NewReader(markup)); link[0] != expected {
...
`
	...
	if links, _ := Parse(strings.NewReader(markup)); !slicesEqual(expected, links) {
